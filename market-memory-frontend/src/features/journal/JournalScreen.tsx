...
export function JournalScreen(){
  ...
  const load=useCallback(async()=>{setLoading(true);try{setEntries(await apiRequest<Entry[]>('/api/journal'));}catch(e){setEntries([]);setMessage(e instanceof Error?e.message:'Unable to load journal.');}finally{setLoading(false);}},[]);
  useFocusEffect(useCallback(()=>{setReview(null);void load();},[load]));
  ...
  return <ScrollView style={s.page} contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
    <View style={s.header}><View><Text style={s.eyebrow}>DECIDE → REVIEW</Text><Text style={s.title}>Journal</Text><Text style={s.sub}>Your thinking, preserved. Return to it with fresh evidence.</Text></View><View style={s.count}><Text style={s.countValue}>{entries.length}</Text><Text style={s.countLabel}>decisions</Text></View></View>
    ...
    {!loading&&!entries.length?<Text style={s.empty}>Start with one thought about an asset. What do you expect, and what could change your mind?</Text>:null}
