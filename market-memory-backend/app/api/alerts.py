...
async def create_alert(payload: AlertCreate, user=Depends(get_current_user)):
    ...
    try:
        data = db_for(user).table("price_alerts").insert(row).execute().data or []
        if not data:
...
async def list_alerts(user=Depends(get_current_user)):
    return db_for(user).table("price_alerts").select("*").eq("user_id", user.id).order("created_at", desc=True).execute().data or []


@router.post("/alerts/check")
async def check_alerts(user=Depends(get_current_user)):
    alerts = (
        db_for(user).table("price_alerts")
        .select("*")
        .eq("user_id", user.id)
        .eq("is_active", True)
...
async def delete_alert(alert_id: int, user=Depends(get_current_user)):
    db_for(user).table("price_alerts").delete().eq("id", alert_id).eq("user_id", user.id).execute()
