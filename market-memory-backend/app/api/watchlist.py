...
async def add_watchlist(payload: WatchlistCreate, user=Depends(get_current_user)):
    ...
    try:
        data = db_for(user).table("watchlist_items").insert(row).execute().data or []
...
async def list_watchlist(user=Depends(get_current_user)):
    return db_for(user).table("watchlist_items").select("*").eq("user_id", user.id).order("created_at", desc=True).execute().data or []
...
async def delete_watchlist(item_id: int, user=Depends(get_current_user)):
    db_for(user).table("watchlist_items").delete().eq("id", item_id).eq("user_id", user.id).execute()
